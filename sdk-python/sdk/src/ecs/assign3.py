...
class assign3:
    ...
    # 再分配，尽量装满每个服务器
    def Reassign(self, Sorted_Vm, pre, SortByflavor):
        for ser in self.Servers:
            for i in range(len(Sorted_Vm) - 20, -1, -1):
    ...
    def sort(self, input_txt, pre):
        ...
        for flavor in pre:
            num = pre.get(flavor)
            SortByflavor.append([flavor, int(input_txt[flavor_id.get(flavor)][1]), int(input_txt[flavor_id.get(flavor)][2])
                                 / 1024, float(self.sum_cpu) % int(input_txt[flavor_id.get(flavor)][1])])
            for i in range(num):
                cpu = int(input_txt[flavor_id.get(flavor)][1])
                mem = int(input_txt[flavor_id.get(flavor)][2]) / 1024
                if self.OptimizaType == 'CPU'or self.OptimizaType == 'cpu':
                    temp.append([flavor, cpu, mem, float(cpu) / float(mem), abs(float(self.sum_cpu) / float(self.sum_mem) - float(cpu) /
                                                                                float(mem))])
                else:
                    temp.append([flavor, cpu, mem, float(mem) / float(cpu), abs(float(self.sum_cpu) / float(self.sum_mem) - float(cpu) /
                                                                                float(mem))])
        # print SortByflavor
    ...
    def getMinratio_flavor(self, sortup):
        if self.Servers.__len__() == 0 or self.Servers[-1].cpu == 0 or self.Servers[-1].mem == 0:
            sum_cpu = self.sum_cpu
            sum_mem = self.sum_mem
        else:
            sum_cpu = self.Servers[-1].cpu
            sum_mem = self.Servers[-1].mem
        if self.OptimizaType == 'CPU' or self.OptimizaType == 'cpu':
            ser_ratio = float(sum_cpu) / float(sum_mem)
        else:
            ser_ratio = float(sum_mem) / float(sum_cpu)
        temp = []
        for index, item in enumerate(sortup):
            temp.append([index, item[0], item[1], item[2], item[3], abs(ser_ratio - item[3])])
        index, flavor, need_cpu, need_mem, ratio, _ = min(temp, key=lambda x: x[5])
        index = temp.index([index, flavor, need_cpu, need_mem, ratio, _])
        sortup.pop(index)
        # print flavor,need_cpu,need_mem
        return sortup, flavor, need_cpu, need_mem
    ...
    def assign(self, input_txt, pre):
        ...
        while sortup.__len__() != 0:
            # print 'sortup len',len(sortup)
            flag = False
            sortup, flavor, Need_cpu, Need_mem = self.getMinratio_flavor(sortup)
        ...
        # 尽量装满每个服务器
        sortup = sorted(sortdown, key=lambda x: x[4])
        for i in range(20):
            pre = self.Reassign(sortup, pre, SortByflavor)
        # print '分配后的结果：'
        # for ser in self.Servers:
        #     print ser.VM
