...
class assign:
    ...
    def sort(self, input_txt, pre):
        ...
        for flavor in pre:
            num = pre.get(flavor)
            SortByflavor.append([flavor, int(input_txt[flavor_id.get(flavor)][1]), int(input_txt[flavor_id.get(flavor)][2]) / 1024])
            for i in range(num):
                temp.append([flavor, int(input_txt[flavor_id.get(flavor)][1]), int(input_txt[flavor_id.get(flavor)][2]) / 1024])
        # print SortByflavor
        SortByflavor = sorted(SortByflavor, key=lambda x: x[1], reverse=True)
        print 'SortByflavor', SortByflavor

        # sored = sorted(temp, key=lambda x: x[2], reverse=True)  # 很奇怪这里，全部按CPU分配更好
    ...
    def assign(self, input_txt, pre):
        Sorted_Vm, SortByflavor = self.sort(input_txt, pre)
        max_num_flavor = max(pre, key=lambda x: pre.get(x))
        ...
        print 'Sorted_Vm', Sorted_Vm
        server = self.NewSer()
        self.Servers.append(server)

        # 装箱
        flag = False
        for flavor, Need_cpu, Need_mem in Sorted_Vm:
            flag = False
        ...
        # 尽量装满每个服务器
        for i in range(20):
            pre = self.Reassign(Sorted_Vm, pre)
        # print '分配后的结果：'
        # for ser in self.Servers:
        #     print ser.VM
